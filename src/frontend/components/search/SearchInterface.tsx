...
export default function SearchInterface() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const filtered = all.filter((r: any) => r.ingredients.toLowerCase().includes(query.toLowerCase()));
      setResults(filtered);
    } catch (e) {
      console.error(e);
    }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {results.length > 0 ? results.map((recipe: any) => (
          ...
          <div className="text-muted-foreground col-span-full">No results yet. Enter ingredients to search.</div>
        )}
      </div>
    </div>
  );
