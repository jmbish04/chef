...
```jsonc
{
  ...
  "durable_objects": {
    "bindings": [
      { "name": "KitchenAgent", "class_name": "KitchenOrchestrator" }
    ]
  },
  "migrations": [
    { "tag": "v1", "new_sqlite_classes": ["KitchenAgent"] }
  ],
  "vars": {
...
### 6.3 Common Queries
* *Verification*: `SELECT * FROM recipes WHERE is_favorite = 1;`
* *RAG Context*: `SELECT * FROM appliances WHERE type = 'Air Fryer';`
